...
void Adc_ConvComplete_ISR(void) interrupt 10
{
   ...
   // *** BEGIN EXTRA CODE ADDED TO TESTPANEL2 *********>
   // Toggle In_Packet[6] value (to make a button blink in the Testpanel SW)
   // This is to demonstrate that the TestPanel FW version has changed
   if(In_Packet[6] == 1)
      In_Packet[6] = 2;
   else
      In_Packet[6] = 1;
   // *** END EXTRA CODE ADDED TO TESTPANEL2 ***********<
